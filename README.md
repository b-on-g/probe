...
- **Пропуск решает не тест, а `$bog_probe_done( out, ...ок )`.** Сначала он смотрит на `$bog_probe_skip`: объявлен ли браузер обязательным переменной `CI` (`$bog_probe_need`) — объявлен, падает с внятной причиной; не объявлен, печатает строку про пропуск и пропускает. Только потом сверяет успешные строки. Порядок именно такой: `$bog_probe_skip` в выводе значит, что проба не делала того, о чём отчитывается, даже если рядом напечатана успешная строка другого шага. Сравнивать `out.includes( $bog_probe_skip )` в тесте больше не нужно: без объявления пропуск был зелёным везде, и пропавший Chrome делал зелёным весь набор проб.
- **Пропуск пропуску не равен, и разница не в строке, а в природе условия.** «Chrome не найден» это `$bog_probe_skip`, и в объявленном окружении он красный. Свои пропуски проверки — другие строки, и они бывают двух родов.
	- **Непреодолимый:** в этом окружении проверка невозможна по природе, никакой настройкой прогона её проверяемой не сделать. Подпись пиксельной картинки на чужом рендерере такая: другой бэкенд рисует другие числа, сверять их не с чем. Такой пропуск честен, передаётся в `$bog_probe_done` успешной строкой и остаётся зелёным.
	- **Недостающее условие:** проверка возможна в принципе, просто условия нет — мастер Базы не поднят, сервис не запущен. Успехом это быть не может: прогон обещает проверку и не делает её. Но и вечно красным делать нельзя, постоянный красный через день становится фоном и маскирует следующий настоящий. Правильный ход — **убрать ложное обещание**: держать такую проверку отдельным модулем и НЕ вписывать его в `modules:` рабочего потока, а рядом написать, чем поднять условие и как прогнать руками. Тогда ложного зелёного нет, потому что теста в прогоне нет; внутри такого модуля своя строка пропуска снова честна, ведь модуль никто не называет проверкой CI.
- **Списка приемлемых пропусков в помощнике нет намеренно.** Приемлемость решается не перечислением строк, а тем, преодолимо ли условие. Список превратил бы решение в инерцию: первый неочевидный случай попал бы в него просто потому, что список есть.
